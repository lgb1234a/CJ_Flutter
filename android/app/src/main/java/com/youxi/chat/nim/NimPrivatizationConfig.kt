...
object NimPrivatizationConfig {
    ...
    // 音视频加密
    private const val KEY_NRTC_STREAM_ENC_TYPE = "nrtc_stream_enc_type"
    private const val KEY_NRTC_STREAM_ENC_KEY = "nrtc_stream_enc_key"
    // 握手协议(国密)
    ...
    fun enablePrivateConfig(enable: Boolean, context: Context) {
    ...
    private fun parseAddresses(jsonObject: JSONObject?): ServerAddresses? {
        ...
        addresses.handshakeType = NimHandshakeType.value(jsonObject.optInt(KEY_HAND_SHAKE_TYPE, NimHandshakeType.V1.value))
        ...
        addresses.negoKeyNeca = AsymmetricType.value(jsonObject.optInt(KEY_NEGO_KEY_NECA, AsymmetricType.RSA.value))
        ...
        addresses.commEnca = SymmetryType.value(jsonObject.optInt(KEY_COMM_ENCA, SymmetryType.RC4.value))
        addresses.linkIpv6 = jsonObject.optString(KEY_LINK_IPV6)
        addresses.ipProtocolVersion = IPVersion.value(jsonObject.optInt(KEY_IP_PROTOCOL_VERSION, IPVersion.IPV4.value))
    ...
    private fun checkValid(addresses: ServerAddresses) {
        ...
        require(!(addresses.handshakeType == NimHandshakeType.V1 && addresses.module == null)) { "ServerAddresses handshakeType is V1, but module is null" }
        require(!(addresses.handshakeType == NimHandshakeType.V0 && (addresses.negoKeyEncaKeyParta == null || addresses.negoKeyEncaKeyPartb == null))) { "ServerAddresses handshakeType is V0, but negoKeyEncaKeyParta is not null or negoKeyEncaKeyPartb  is null" }
    }
    ...
    private fun autoAdjust(addresses: ServerAddresses) {
        addresses.module = if (TextUtils.isEmpty(addresses.module)) null else addresses.module
        addresses.lbs = if (TextUtils.isEmpty(addresses.lbs)) null else addresses.lbs
        addresses.defaultLink = if (TextUtils.isEmpty(addresses.defaultLink)) null else addresses.defaultLink
        addresses.nosUploadLbs = if (TextUtils.isEmpty(addresses.nosUploadLbs)) null else addresses.nosUploadLbs
        addresses.nosUploadDefaultLink = if (TextUtils.isEmpty(addresses.nosUploadDefaultLink)) null else addresses.nosUploadDefaultLink
        addresses.nosUpload = if (TextUtils.isEmpty(addresses.nosUpload)) null else addresses.nosUpload
        addresses.nosDownloadUrlFormat = if (TextUtils.isEmpty(addresses.nosDownloadUrlFormat)) null else addresses.nosDownloadUrlFormat
        addresses.nosDownload = if (TextUtils.isEmpty(addresses.nosDownload)) null else addresses.nosDownload
        addresses.nosAccess = if (TextUtils.isEmpty(addresses.nosAccess)) null else addresses.nosAccess
        addresses.ntServerAddress = if (TextUtils.isEmpty(addresses.ntServerAddress)) null else addresses.ntServerAddress
        addresses.nrtcStreamEncKey = if (TextUtils.isEmpty(addresses.nrtcStreamEncKey)) null else addresses.nrtcStreamEncKey
        addresses.negoKeyEncaKeyParta = if (TextUtils.isEmpty(addresses.negoKeyEncaKeyParta)) null else addresses.negoKeyEncaKeyParta
        addresses.negoKeyEncaKeyPartb = if (TextUtils.isEmpty(addresses.negoKeyEncaKeyPartb)) null else addresses.negoKeyEncaKeyPartb
        addresses.linkIpv6 = if (TextUtils.isEmpty(addresses.linkIpv6)) null else addresses.linkIpv6
        addresses.probeIpv4Url = if (TextUtils.isEmpty(addresses.probeIpv4Url)) null else addresses.probeIpv4Url
        addresses.probeIpv6Url = if (TextUtils.isEmpty(addresses.probeIpv6Url)) null else addresses.probeIpv6Url
