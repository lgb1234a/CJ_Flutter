...
object NimSdkOptionConfig {
    fun getSDKOptions(context: Context): SDKOptions? {
        ...
        // 通知栏显示用户昵称和头像
        options.userInfoProvider = NimUserInfoProvider(NimCache.getContext())
    ...
    val TEST_NOS_SCENE_KEY = "test_nos_scene_key"
...
// 该存储区在API 19以上不需要写权限，即可配置 <uses-permission android:name="android.permission.WRITE_EXTERNAL_STORAGE" android:maxSdkVersion="18"/>
        ...
        if (TextUtils.isEmpty(storageRootPath)) { // SD卡应用公共存储区(APP卸载后，该目录不会被清除，下载安装APP后，缓存数据依然可以被加载。SDK默认使用此目录)，该存储区域需要写权限!
            storageRootPath = Environment.getExternalStorageDirectory().toString() + "/" + NimCache.getContext().getPackageName()
        }
    ...
    private fun initStatusBarNotificationConfig(options: SDKOptions) { // load 应用的状态栏配置
        val config = loadStatusBarNotificationConfig()
        // load 用户的 StatusBarNotificationConfig 设置项
        var userConfig: StatusBarNotificationConfig? = UserPreferences.statusConfig
        if (userConfig == null) {
            userConfig = config
        } else {
    ...
    private fun loadStatusBarNotificationConfig(): StatusBarNotificationConfig {
        ...
        config.notificationSmallIconId = R.drawable.small_app_icon
        config.notificationColor = NimCache.getContext().getResources().getColor(R.color.color_blue_3a9efb)
        // 通知铃声的uri字符串
        config.notificationSound = "android.resource://${NimCache.getContext().packageName}/raw/msg"
    ...
    private val messageNotifierCustomization: MessageNotifierCustomization = object : MessageNotifierCustomization {
    ...
    private fun buildMixPushConfig(): MixPushConfig? { // 第三方推送配置
        ...
        // 小米推送
        config.xmAppId = "2882303761517909572"
        config.xmAppKey = "5411790967572"
        config.xmCertificateName = "xiaomipush"
        // 华为推送
...
//        config.vivoCertificateName = "DEMO_VIVO_PUSH"
        // oppo推送
        config.oppoAppId = "3722185"
        config.oppoAppKey = "f5ac5269547e46b980e70b8d6d6c5e4c"
        config.oppoAppSercet = "91e56285fcf14b6c9e9002bfbcedf412"
        config.oppoCertificateName = "oppopush"
        return config
    }
}