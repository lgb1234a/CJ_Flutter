...
import com.youxi.chat.module.main.reminder.ReminderSettings;
...
public class PagerSlidingTabStrip extends HorizontalScrollView implements OnPageChangeListener {
    ...
    public void onPageSelected(int position) {
        setChooseTabViewStyle(position);
    }
    ...
    public void notifyDataSetChanged() {
        ...
        SlidingTabPagerAdapter adapter = (SlidingTabPagerAdapter) pager.getAdapter();

        tabCount = adapter.getCount();

        for (int i = 0; i < tabCount; i++) {
            addTabView(i, adapter.getPageTitle(i).toString(), adapter.getPageIcon(i));
        }
        ...
        getViewTreeObserver().addOnGlobalLayoutListener(new OnGlobalLayoutListener() {
            ...
            public void onGlobalLayout() {
                ...
                setChooseTabViewStyle(currentPosition);
    ...
    private void setChooseTabViewStyle(int position) {
        ...
        for (int i = 0; i < childCount; ++i) {
			...
			imageView = (ImageView) tabView.findViewById(R.id.tab_title_icon);
            if (i == position) {
    ...
    private void addTabView(final int position, String title, int icon) {
        View tabView = null;
        ...
        // 添加Tab图标功能
		ImageView iconIV = tabView.findViewById(R.id.tab_title_icon);
		if (iconIV != null) {
    ...
    public void updateTab(int index, ReminderItem item) {
        ...
        if (unread > 0) {
            unreadTV.setText(String.valueOf(ReminderSettings.INSTANCE.unreadMessageShowRule(unread)));
        }
    }
