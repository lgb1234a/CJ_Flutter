...
#import "MFGetCustomerInfo.h"
#import "MFPacketModel.h"

typedef NS_ENUM(NSUInteger, MFPacketStatusType) {
    MFPacketCanGet = 0,   /**< 可领取 */
    MFPacketIsGet,        /**< 已领取 */
    MFPacketIsDue,        /**< 红包失效 */
    MFPacketIsNull,       /**< 手慢了，红包被抢完了 */
    MFPacketIsCommon,     /**< 普通红包，自己不可抢 */
    MFPacketUn,           /**< 未知，获取红包信息失败 */
};

typedef enum MFTransferStatus {
    kMFTrfStatusGet = 1,         // 已领取
    kMFTrfStatusReturn = 2,      // 退还
}MFTransferStatus;

UIKIT_EXTERN NSString * const kMFPacketShow;/**< 界面出现 */
UIKIT_EXTERN NSString * const kMFPacketHide;/**< 界面消失 */
...
/**
 ...
 @param model 红包信息，详细查看 MFPacketModel
 @param status 发送状态
 */
...
/**
 ...
 @param type 红包类型，详见 MFPacketStatusType 调拆包的时候才有此回调
 */
...
@protocol MFManagerTrfDelegate <NSObject>
 ...
 */
- (void)doMFTransferAccountsWithReceivedUserID:(NSString *)receiveUid Message:(NSString *)msg transferOrder:(NSString *)transferId Amount:(NSString *)amountStr;
 ...
 */
- (void)doMFTransferReceiptStatuaWithTransferOrder:(NSString *)transferOrder TransferOperateStatus:(MFTransferStatus)transferStat Amount:(NSString *)amountStr;

@end

@interface MFPacket : NSObject

@property (nonatomic, weak) id <MFManagerDelegate> delegate;
@property (nonatomic, weak) id <MFManagerTrfDelegate> trfDelegate;
 ...
 */
+ (void)instanceMFPacketWithPartnerId:(NSString *)partnerId EnvelopeName:(NSString *)envName aliPaySchemeUrl:(NSString *)aliPayscheme weChatSchemeUrl:(NSString *)weChatScheme appMothod:(BOOL)isOnLine;

/**
 MFSDK 注册方法 dynamicToken+classname
 
 @param partnerId            渠道名称（我们公司分配给你们的渠道字符串）
 @param envName              红包名称
 @param aliPaySchemeUrl      支付宝回调Scheme【保证格式的正确性】
 @param weChatSchemeUrl      微信回调Scheme
 @param className            外部获取用户信息的类 需遵循 MFGetCustomerInfo 协议
 @param isOnLine             是否正式环境 YES：正式环境    NO：测试环境
 */
+ (void)instanceMFPacketWithPartnerId:(NSString *)partnerId EnvelopeName:(NSString *)envName aliPaySchemeUrl:(NSString *)aliPayscheme weChatSchemeUrl:(NSString *)weChatScheme customerClass:(NSString *)className dynamicToken:(BOOL)dynamicToken appMothod:(BOOL)isOnLine;
